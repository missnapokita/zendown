(function () {
  ...
  function connectionValues() {
    return normalizeConnectionValues({
      ...
      branch: byId("github-branch").value.trim(),
  ...
  function normalizeConnectionValues(values) {
    var repoText = String(values.repo || "").trim()
      .replace(/^https?:\/\/github\.com\//i, "")
      .replace(/^github\.com\//i, "")
      .replace(/^\/+|\/+$/g, "")
      .replace(/\.git$/i, "");
    var parts = repoText.split("/").filter(Boolean);
    if (parts.length >= 2) {
      values.owner = parts[0];
      values.repo = parts[1];
    } else {
      values.repo = repoText;
    }
    values.owner = String(values.owner || "").trim().replace(/^@/, "");
    values.branch = String(values.branch || "").trim();
    values.path = String(values.path || "apps.json").trim().replace(/^\/+/, "") || "apps.json";
    byId("github-owner").value = values.owner;
    byId("github-repo").value = values.repo;
    byId("github-path").value = values.path;
    return values;
  }
  ...
  function showConnectionDiagnostic(title, message, isError) {
    var box = byId("connection-diagnostic");
    box.hidden = false;
    box.classList.toggle("is-error", Boolean(isError));
    box.querySelector(".diagnostic-icon").textContent = isError ? "!" : "✓";
    byId("diagnostic-title").textContent = title;
    byId("diagnostic-message").textContent = message;
  }
  ...
  async function getAuthenticatedUser(token) {
    var response = await fetch("https://api.github.com/user", { headers: apiHeaders(token), cache: "no-store" });
    if (!response.ok) throw await responseError(response, "The GitHub token is invalid or expired");
    return response.json();
  }

  async function getRepository(repoUrl, token) {
    var response = await fetch(repoUrl, { headers: apiHeaders(token), cache: "no-store" });
    if (!response.ok) {
      if (response.status === 404) {
        var notFound = new Error("Repository not found. Check the owner/repository name and make sure the token is allowed to access this repository. (HTTP 404)");
        notFound.status = 404;
        throw notFound;
      }
      throw await responseError(response, "Repository connection failed");
    }
    var repository = await response.json();
    if (repository.archived) throw new Error("This repository is archived and cannot accept Zenhub updates.");
    if (repository.permissions && repository.permissions.push === false) {
      throw new Error("This GitHub account does not have write access to the selected repository.");
    }
    return repository;
  }

  async function resolveBranch(repoUrl, values, repository) {
    var response = await fetch(repoUrl + "/branches?per_page=100", {
      headers: apiHeaders(values.token),
      cache: "no-store"
    });
    if (!response.ok) throw await responseError(response, "Could not read repository branches");
    var branches = await response.json();
    if (!Array.isArray(branches) || branches.length === 0) {
      throw new Error("This repository has no branch yet. Upload the Zenhub website files to the repository first.");
    }

    var names = branches.map(function (branch) { return branch.name; });
    var requested = values.branch;
    var resolved = names.indexOf(requested) !== -1 ? requested : "";
    if (!resolved && repository.default_branch && names.indexOf(repository.default_branch) !== -1) resolved = repository.default_branch;
    if (!resolved) resolved = names[0];
    var changed = requested !== resolved;
    values.branch = resolved;
    byId("github-branch").value = resolved;
    return { name: resolved, changed: changed, requested: requested };
  }

  async function getContents(repoUrl, path, branch, token) {
    var url = repoUrl + "/contents/" + formatApiPath(path) + "?ref=" + encodeURIComponent(branch);
    return fetch(url, { headers: apiHeaders(token), cache: "no-store" });
  }

  function chooseAppsPath(tree, requestedPath) {
    var files = tree && Array.isArray(tree.tree) ? tree.tree : [];
    var matches = files.filter(function (item) {
      return item.type === "blob" && /(^|\/)apps\.json$/i.test(item.path || "");
    }).map(function (item) { return item.path; });
    if (!matches.length) return "";
    var exact = matches.filter(function (path) { return path.toLowerCase() === requestedPath.toLowerCase(); })[0];
    if (exact) return exact;
    matches.sort(function (a, b) { return a.split("/").length - b.split("/").length || a.length - b.length; });
    return matches[0];
  }

  async function locateDataFile(repoUrl, values) {
    var response = await getContents(repoUrl, values.path, values.branch, values.token);
    if (response.ok) return { response: response, pathChanged: false, missing: false };
    if (response.status !== 404) throw await responseError(response, "Could not load the data file");

    var treeUrl = repoUrl + "/git/trees/" + encodeURIComponent(values.branch) + "?recursive=1";
    var treeResponse = await fetch(treeUrl, { headers: apiHeaders(values.token), cache: "no-store" });
    if (treeResponse.ok) {
      var detectedPath = chooseAppsPath(await treeResponse.json(), values.path);
      if (detectedPath) {
        values.path = detectedPath;
        byId("github-path").value = detectedPath;
        var detectedResponse = await getContents(repoUrl, detectedPath, values.branch, values.token);
        if (detectedResponse.ok) return { response: detectedResponse, pathChanged: true, missing: false };
      }
    }
    return { response: response, pathChanged: false, missing: true };
  }

  async function connectGithub(event) {
    event.preventDefault();
    var values = connectionValues();
    if (!values.owner || !values.repo || !values.path || !values.token) {
    ...
    try {
      ...
      var user = await getAuthenticatedUser(values.token);
      var repository = await getRepository(repoUrl, values.token);
      var branchResult = await resolveBranch(repoUrl, values, repository);
      var located = await locateDataFile(repoUrl, values);

      if (!located.missing) {
        var fileData = await located.response.json();
        ...
        fileSha = null;
        showToast("Connected. Publishing will create " + values.path + ".");
      }

      var adjustments = [];
      if (branchResult.changed) adjustments.push("branch corrected to " + values.branch);
      if (located.pathChanged) adjustments.push("data file found at " + values.path);
      var detail = values.owner + "/" + values.repo + " · " + values.branch + " · " + values.path;
      if (adjustments.length) detail += " (" + adjustments.join("; ") + ")";
      if (located.missing) detail += ". The data file will be created on first publish.";
      showConnectionDiagnostic("Connected as @" + (user.login || values.owner), detail, false);
      ...
      fileSha = null;
      updateConnectionUi();
      showConnectionDiagnostic("Connection needs attention", error.message || "Connection failed.", true);
  ...
  function disconnectGithub() {
    ...
    byId("connection-diagnostic").hidden = true;
    updateConnectionUi();
  ...
  async function publishResponseError(response, values) {
    var base = await responseError(response, "Publish failed");
    var error = base;
    if (response.status === 404) {
      error = new Error("GitHub returned 404 for " + values.owner + "/" + values.repo + ". Recreate or edit the token so this exact repository is selected and Contents is set to Read and write, then reconnect. Also confirm branch " + values.branch + " and path " + values.path + ".");
    } else if (response.status === 403) {
      error = new Error("GitHub blocked the publish request. The token needs Contents: Read and write for this repository, and the account must have write access.");
    } else if (response.status === 409) {
      error = new Error("The repository changed after you connected. Reconnect to load the newest apps.json, then publish again.");
    } else if (response.status === 422) {
      error = new Error("GitHub rejected the update. Check that the branch exists and the apps.json path is correct, then reconnect.");
    }
    error.status = response.status;
    return error;
  }

  async function refreshPublishTarget(repoUrl, values) {
    var repository = await getRepository(repoUrl, values.token);
    var branchResult = await resolveBranch(repoUrl, values, repository);
    var located = await locateDataFile(repoUrl, values);
    if (!located.missing) {
      var current = await located.response.json();
      if (fileSha && current.sha && current.sha !== fileSha) {
        var conflict = new Error("apps.json changed after this admin session connected. Reconnect first so no newer changes are overwritten.");
        conflict.status = 409;
        throw conflict;
      }
      fileSha = current.sha || fileSha;
    } else {
      fileSha = null;
    }
    return { branchChanged: branchResult.changed, pathChanged: located.pathChanged };
  }
  ...
  async function publishConfig() {
    ...
    var values = connectionValues();
    if (!values.token) {
      showToast("The GitHub token is missing. Reconnect first.", true);
      return;
    }

    setButtonLoading(byId("publish-button"), true, "Checking target…");
    try {
      var repoUrl = "https://api.github.com/repos/" + encodeURIComponent(values.owner) + "/" + encodeURIComponent(values.repo);
      var target = await refreshPublishTarget(repoUrl, values);
      ...
      if (!response.ok) throw await publishResponseError(response, values);
      ...
      var targetMessage = values.owner + "/" + values.repo + " · " + values.branch + " · " + values.path;
      if (target.branchChanged || target.pathChanged) targetMessage += " (target corrected automatically)";
      showConnectionDiagnostic("Publish target verified", targetMessage, false);
      showToast("Zenhub changes published successfully.");
    } catch (error) {
      var message = error.message || "Publish failed. Reconnect and try again.";
      showConnectionDiagnostic("Publish failed", message, true);
