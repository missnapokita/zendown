(function () {
  ...
  var config = window.STORE_CONFIG || { name: "My Apps", developer: "Developer", apps: [] };
  ...
  function renderFeatured() {
    ...
    featuredApp.style.setProperty("--featured-accent", app.accent || "#64b553");
    featuredApp.innerHTML =
      '<div class="hero-copy">' +
        '<p class="eyebrow">Featured app</p>' +
        '<h1>' + escapeHtml(app.tagline || app.name) + "</h1>" +
  ...
  function downloadApp(app) {
    if (!app) return;
    if (!app.apkUrl) {
      showToast("APK link for " + app.name + " is not set yet. Add it in apps.js.");
      return;
    }
